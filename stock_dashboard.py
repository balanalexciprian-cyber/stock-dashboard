...
def get_data(tick):
    try:
        time.sleep(0.6)   # delay ca să nu mai dea rate limit
...
for group_name, ticks in groups.items():
    ...
    for i, tick in enumerate(ticks):
        current, hist = get_data(tick)
        if current:
            # Calcul procent simplu (față de ultimul preț cunoscut)
            change_pct = 0.0
            emoji = "🟢"
            ...
            with cols[i % 4]:
                ...
                if group_name != "🤖 AI TECH":
                    ...
                    current_val = amt_per * 1.0   # temporar
...
st.caption("Date de la Yahoo Finance • Varianta completă optimizată")
